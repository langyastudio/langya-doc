...
- Authentication（认证）是验证您的**身份**的凭据（例如用户名/用户 ID 和密码），通过这个凭据，系统得以知道你就是你，也就是说系统存在你这个用户。所以，Authentication 被称为身份/用户验证。
- Authorization（授权）发生在 Authentication（认证）之后。授权嘛，光看意思大家应该就明白，它主要掌管我们访问系统的**权限**。比如有些特定资源只能具有特定权限的人才能访问比如 admin，有些对系统资源操作比如删除、添加、更新只能特定人才具有。

认证 ：

![](https://img-note.langyastudio.com/202111171131044.png?x-oss-process=style/watermark)

授权：

![](https://img-note.langyastudio.com/202111171131185.png?x-oss-process=style/watermark)
...
![RBAC](https://img-note.langyastudio.com/202111171131061.png?x-oss-process=style/watermark)

在 RBAC 中，权限与角色相关联，用户通过成为适当角色的成员而得到这些角色的权限。这就极大地**简化了权限的管理。**

本系统的权限设计相关的表如下（一共 5 张表，2 张用户建立表之间的联系）：

![](https://img-note.langyastudio.com/202111171131990.png?x-oss-process=style/watermark)

通过这个权限模型，我们可以创建不同的角色并为不同的角色分配不同的权限范围（菜单）。

![](https://img-note.langyastudio.com/202111171115558.png?x-oss-process=style/watermark)
...
![](https://img-note.langyastudio.com/202111171115678.png?x-oss-process=style/watermark)
...
![](https://img-note.langyastudio.com/202111231614738.png?x-oss-process=style/watermark)
...
![](https://img-note.langyastudio.com/202111171136997.png?x-oss-process=style/watermark)
...
![jwt](https://img-note.langyastudio.com/202111171136016.png?x-oss-process=style/watermark)
...
SSO(Single Sign On)即单点登录说的是**用户登陆多个子系统的其中一个就有权访问与其相关的其他系统**。举个例子我们在登陆了京东金融之后，我们同时也成功登陆京东的京东超市、京东国际、京东生鲜等子系统。

![sso](https://img-note.langyastudio.com/202111231613156.png?x-oss-process=style/watermark)



## OAuth 2.0

OAuth 是一个行业的标准授权协议，主要用来授权**第三方应用获取有限的权限**。而 OAuth 2.0 是对 OAuth 1.0 的完全重新设计，OAuth 2.0 更快，更容易实现，OAuth 1.0 已经被废弃。详情请见：[rfc6749](https://tools.ietf.org/html/rfc6749)。
...
![](https://img-note.langyastudio.com/202111171136375.png?x-oss-process=style/watermark)



下图是 [Slack OAuth 2.0 第三方登录](https://api.slack.com/legacy/oauth)的示意图：

![](https://img-note.langyastudio.com/202111171136469.png?x-oss-process=style/watermark)
