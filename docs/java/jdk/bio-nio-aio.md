...
![传统BIO通信模型图](https://img-note.langyastudio.com/202110191252468.png?x-oss-process=style/watermark)
...
![伪异步IO模型图](https://img-note.langyastudio.com/202110191252958.png?x-oss-process=style/watermark)
...
下面代码中演示了 BIO 通信（一请求一应答）模型。我们会在客户端创建多个线程依次连接服务端并向其发送"当前时间+:hello world"，服务端会为每个客户端线程创建一个线程来处理。
...
NIO 是一种同步非阻塞的I/O模型，在Java 1.4 中引入了 NIO 框架，对应 java.nio 包，提供了 Channel , Selector，Buffer等抽象。
...
Buffer 是一个对象，它包含一些要写入或者要读出的数据。在 NIO 类库中加入 Buffer 对象，体现了新库与原I/O的一个重要区别。在面向流的 I/O 中可以将数据直接写入或者将数据直接读到 Stream 对象中。虽然 Stream 中也有 Buffer 开头的扩展类，但只是流的包装类，还是从流读到缓冲区，而 NIO 却是直接读到 Buffer 中进行操作。
...
![一个单线程中Selector维护3个Channel的示意图](https://img-note.langyastudio.com/202110191255769.png?x-oss-process=style/watermark)
...
![NIO读写数据的方式](https://img-note.langyastudio.com/202110191255557.png?x-oss-process=style/watermark)
...
整个 NIO 体系包含的类远远不止这三个，只能说这三个是 NIO 体系的“核心API”。我们上面已经对这三个概念进行了基本的阐述，这里就不多做解释了。
