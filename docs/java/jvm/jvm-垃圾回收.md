...
![](https://img-note.langyastudio.com/202111091447979.png?x-oss-process=style/watermark)
...
Java 的自动内存管理**主要是针对对象内存的分配和回收**。同时 Java 自动内存管理最核心的功能是 **堆** 内存中对象的分配与回收。

Java 堆是垃圾收集器管理的主要区域，因此也被称作**GC 堆（Garbage Collected Heap）**。从垃圾回收的角度，由于现在收集器基本都采用分代垃圾收集算法，所以 Java 堆还可以细分为：新生代和老年代：再细致一点有：Eden 空间、From Survivor、To Survivor 空间等。**进一步划分的目的是更好地回收内存或者更快地分配内存。**

**堆空间的基本结构：**

![](https://img-note.langyastudio.com/202111091447909.png?x-oss-process=style/watermark)
...
```
![堆内存常见分配策略 ](https://img-note.langyastudio.com/202111091447100.png?x-oss-process=style/watermark)
...
通过以下方式运行：
![](https://img-note.langyastudio.com/202111091447348.png?x-oss-process=style/watermark)

添加的参数：`-XX:+PrintGCDetails`
![](https://img-note.langyastudio.com/202111091447693.png?x-oss-process=style/watermark)

运行结果 (红色字体描述有误，应该是对应于 JDK1.7 的永久代)：

![](https://img-note.langyastudio.com/202111091449388.jpeg?x-oss-process=style/watermark)
...
![](https://img-note.langyastudio.com/202111091449554.jpeg?x-oss-process=style/watermark)
...
大对象就是需要大量连续内存空间的对象（比如：字符串、数组）。为了避免为大对象分配内存时由于**分配担保机制带来的复制而降低效率**。
...
>
> ![](https://img-note.langyastudio.com/202111171116691.png?x-oss-process=style/watermark)
>
> **动态年龄计算的代码如下：**
>
> ```c++
...
![](https://img-note.langyastudio.com/202111091451695.png?x-oss-process=style/watermark)
...
![](https://img-note.langyastudio.com/202111091447836.png?x-oss-process=style/watermark)
...
这个方法实现简单，效率高，但是目前主流的虚拟机中并没有选择这个算法来管理内存，其最主要的原因是**它很难解决对象之间相互循环引用**的问题。 所谓对象之间的相互引用问题，如下面代码所示：除了对象 objA 和 objB 相互引用着对方之外，这两个对象之间再无任何引用。但是他们因为互相引用对方，导致它们的引用计数器都不为 0，于是引用计数算法无法通知 GC 回收器回收他们。
...
![可达性分析算法 ](https://img-note.langyastudio.com/202111091447833.png?x-oss-process=style/watermark)
...
- JDK1.2 之前
  ...
  Java 中引用的定义很传统：如果 reference 类型的数据存储的数值代表的是另一块内存的起始地址，就称这块内存代表一个引用。

- JDK1.2 以后

  Java 对引用的概念进行了扩充，将引用分为强引用、软引用、弱引用、虚引用四种（引用强度逐渐减弱）
...
![垃圾收集算法分类](https://img-note.langyastudio.com/202111091447730.png?x-oss-process=style/watermark)
...
![](https://img-note.langyastudio.com/202111091447547.jpeg?x-oss-process=style/watermark)
...
![复制算法](https://img-note.langyastudio.com/202111091447524.png?x-oss-process=style/watermark)
...
![标记-整理算法 ](https://img-note.langyastudio.com/202111091447973.png?x-oss-process=style/watermark)
...
![垃圾收集器分类](https://img-note.langyastudio.com/202111091447148.png?x-oss-process=style/watermark)
...
![ Serial 收集器 ](https://img-note.langyastudio.com/202111091447125.png?x-oss-process=style/watermark)
...
![ParNew 收集器 ](https://img-note.langyastudio.com/202111091447651.png?x-oss-process=style/watermark)
...
Parallel Scavenge 收集器关注点是**吞吐量（高效率的利用 CPU）**。所谓吞吐量就是 CPU 中用于运行用户代码的时间与 CPU 总消耗时间的比值。 Parallel Scavenge 收集器提供了很多参数供用户找到最合适的停顿时间或最大吞吐量，如果对于收集器运作不太了解，手工优化存在困难的时候，使用 Parallel Scavenge 收集器配合自适应调节策略，把内存管理优化交给虚拟机去完成也是一个不错的选择。

**新生代采用标记-复制算法，老年代采用标记-整理算法。**

![Parallel Scavenge 收集器 ](https://img-note.langyastudio.com/202111091447365.png?x-oss-process=style/watermark)
...
CMS（Concurrent Mark Sweep）收集器是一种以获取**最短回收停顿时间**为目标的收集器。它非常符合在**注重用户体验**的应用上使用。
...
![CMS 垃圾收集器 ](https://img-note.langyastudio.com/202111091447557.png?x-oss-process=style/watermark)
